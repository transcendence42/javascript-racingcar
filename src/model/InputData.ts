...
class InputData {
  ...
  checkEmptyInput(input: HTMLInputElement): boolean {
    return input.value.trim() === '';
  };

  checkArrayHasEmptyElement(array: Array<string>): boolean {
    return array.some((x) => x === '');
  }

  checkArrayHasOneElement(array: Array<string>): boolean {
    return array.length < 2;
  };

  checkArrayDupElements(array: Array<string>): boolean {
    return array.some((x) => {
      return array.indexOf(x) !== array.lastIndexOf(x);
    });
  };

  checkArrayElementsLength(array: Array<string>): boolean {
  ...
  checkCarNames(carNameInput: HTMLInputElement, carNameArray: Array<string>): boolean {
  ...
  checkTryCount(tryCountInput: HTMLInputElement): boolean {
